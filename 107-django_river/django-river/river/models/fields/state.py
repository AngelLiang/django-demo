...
def _on_workflow_object_saved(sender, instance, created, *args, **kwargs):
    ...
    if instance_class._meta.proxy is True:
        instance_class = instance_class.__base__

    for instance_workflow in instance.river.all(instance_class):
        if created:
            instance_workflow.initialize_approvals()
            if not instance_workflow.get_state():
                init_state = getattr(instance_class.river, instance_workflow.field_name).initial_state
...
def _on_workflow_object_deleted(sender, instance, *args, **kwargs):
    ...
    OnApprovedHook.objects.filter(object_id=instance.pk, content_type=ContentType.objects.get_for_model(instance_class)).delete()
    OnTransitHook.objects.filter(object_id=instance.pk, content_type=ContentType.objects.get_for_model(instance_class)).delete()
    OnCompleteHook.objects.filter(object_id=instance.pk, content_type=ContentType.objects.get_for_model(instance_class)).delete()
