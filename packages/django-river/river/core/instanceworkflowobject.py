...
class InstanceWorkflowObject(object):
    ...
    def jump_to(self, state):
        ...
        try:
            ...
            # 前面的所有跳过的 PENDING 状态的 transition
    ...
    def get_available_approvals(self, as_user=None, destination_state=None):
        """
        使用mysql数据库的时候，调用这个方法不能在事务里面

        > mysql基于GTID复制不支持事务里包含CREAT TEMPORARY TABLE和DROP TEMPORARY TABLE
        """
    ...
    def approve(self, as_user, next_state=None, comment=''):
        ...
        with transaction.atomic():
            ...
            pre_approve.send(self.content_type.model_class(), workflow_object=self.workflow_object, field_name=self.field_name, transition_approval=approval)  # add
            ...
            with self._approve_signal(approval), self._transition_signal(has_transit, approval), self._on_complete_signal():
                self.workflow_object.save()
            post_approve.send(self.content_type.model_class(), workflow_object=self.workflow_object, field_name=self.field_name, transition_approval=approval,  transition_approval_meta=approval.meta)  # add
