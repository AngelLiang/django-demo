...
class TransitionApprovalMeta(BaseModel):
    ...
    workflow = models.ForeignKey(Workflow, verbose_name=_("Workflow"), related_name='transition_approval_metas', on_delete=models.CASCADE)

    transition_meta = models.ForeignKey(TransitionMeta, verbose_name=_("Transition Meta"), related_name='transition_approval_meta', on_delete=models.CASCADE)
    ...
    def natural_key(self):
        return self.workflow.natural_key() + (self.transition_meta.source_state.slug, self.transition_meta.destination_state.slug, self.priority,)
    natural_key.dependencies = ['river.workflow', 'river.transitionmeta']
